...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProducstWithNoQuantityAvailable = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity < product.quantity
    );

    if (findProducstWithNoQuantityAvailable.length) {
      throw new AppError(`The quantity ${findProducstWithNoQuantityAvailable[0].quantity} is not available for ${findProducstWithNoQuantityAvailable}.`)
    }
